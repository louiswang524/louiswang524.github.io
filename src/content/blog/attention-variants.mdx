...
The core idea: instead of caching full $K$ and $V$ matrices, compute a compressed latent $c^{KV} \in \mathbb{R}^{d_c}$ where $d_c \ll n_h \cdot d_h$:

$$c^{KV} = W^{DKV} h$$

At inference, decompress on the fly:

$$K = W^{UK} c^{KV}, \quad V = W^{UV} c^{KV}$$

Only $c^{KV}$ is cached. For DeepSeek-V2, this reduces the KV cache per token by 93.3% compared to standard MHA. GQA with $G = 8$ groups achieves roughly an 8× reduction; MLA achieves roughly a 57–93× reduction depending on configuration.
...
12. DeepSeek-AI, Liu, A., Feng, B., et al. (2024). [DeepSeek-V2: A Strong, Economical, and Efficient Mixture-of-Experts Language Model](https://arxiv.org/abs/2405.04434). *arXiv:2405.04434*.

13. DeepSeek-AI, Liu, A., Feng, B., et al. (2024). [DeepSeek-V3 Technical Report](https://arxiv.org/abs/2412.19437). *arXiv:2412.19437*.

14. Qwen Team, Alibaba Group. (2024). [Qwen2 Technical Report](https://arxiv.org/abs/2407.10671). *arXiv:2407.10671*.

15. Qwen Team, Alibaba Group. (2024). [Qwen2.5 Technical Report](https://arxiv.org/abs/2412.15115). *arXiv:2412.15115*.

16. Kimi Team, Moonshot AI. (2025). [Kimi k1.5: Scaling Reinforcement Learning with LLMs](https://arxiv.org/abs/2501.12599). *arXiv:2501.12599*.
