...
import KBArchitecture from '../../components/diagrams/KBArchitecture.astro';
...
One thing I didn't anticipate when building this: most people already have an Obsidian vault. They've been collecting notes for months or years. Starting a new KB from scratch and ignoring all that felt wrong.

So I added two more skills.

### `/kb-import` — Smart Obsidian Vault Import

This takes an existing Obsidian vault and routes each note into the right place:
...
The classification is judgment-based, not rule-based. A note titled "Attention Mechanism" with structured headings and a clear definition gets treated as a concept article. A note that says "attention paper — read again, connects to RLHF somehow??" gets treated as a raw note. After import, you run `/kb-compile` and the raw notes get processed into the wiki like any other ingested content.

### `/kb-merge-vault` — Combining Two KB Vaults
...
The repo is open source. Nine skills, one setup command:
