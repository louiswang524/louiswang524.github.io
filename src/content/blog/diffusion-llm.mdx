...
LLaDA's answer was yes. The authors trained an 8B-parameter masked diffusion model from scratch, comparable in scale to LLaMA-3 8B. The paper reports strong scalability across broad benchmarks and says LLaDA 8B is competitive with LLaMA3 8B in in-context learning. Older dLLMs had not been demonstrated at this scale. LLaDA materially narrowed that gap.
...
The empirical result: LLaDA-8B is competitive with LLaMA-3-8B in the paper's reported in-context-learning setting, establishing that masked diffusion is not obviously doomed by architecture alone — it had also not been explored seriously enough at scale.
...
**AR-model initialization.** Training a large masked diffusion model from scratch is expensive. Dream addresses part of that cost by starting from a pretrained AR LLM and adapting it to function as a denoiser. The intuition: AR models have already learned rich token-level representations across a large corpus. Those representations are useful regardless of whether generation proceeds left-to-right or via iterative unmasking. The paper presents this as a simple, effective way to improve diffusion-LM training.
...
Together, these two contributions make Dream a stronger and more practical diffusion-LM recipe than earlier straightforward training setups.
...
Mercury ([Khanna et al., Inception Labs, 2025, arXiv:2506.17298](https://arxiv.org/abs/2506.17298)) is the clearest 2025 example of a commercial-scale diffusion LLM. Before Mercury, most dLLM discussion was concentrated in research papers. Mercury made the commercial case concrete.

**What it demonstrates.** Mercury Coder Mini achieves 1,109 tokens/sec on H100. The paper says Mercury Coder Mini and Small achieve state-of-the-art throughput on H100s and outperform speed-optimized frontier models by up to 10x on average while maintaining comparable quality.
...
**Significance.** Mercury moved dLLMs from "this might work at scale" to a much stronger commercial plausibility claim. The benchmark debate will continue, but Mercury made it harder to dismiss diffusion LLMs as purely academic prototypes.
...
**AR vs. MDM controlled comparison** ([Vicentino, 2026, arXiv:2603.22075](https://arxiv.org/abs/2603.22075)) is a useful controlled head-to-head comparison of autoregressive and masked diffusion LMs with matched model size, training data, and compute in a small-scale TinyStories setting. Most prior comparisons did not control all three simultaneously. This work is useful for evaluating quality claims that ignore these controls, but its conclusions should be interpreted within its experimental scale.
...
AR models still lead on most standard benchmarks — perplexity on held-out text, MMLU, HumanEval — at matched parameter counts. That lead has narrowed substantially since 2023. LLaDA-8B ([Nie et al., 2025, arXiv:2502.09992](https://arxiv.org/abs/2502.09992)) is reported as competitive with LLaMA-3-8B in in-context learning at the same scale. "Competitive" is not "equal" — but it is no longer the 1000x compute gap from early continuous diffusion work.

Before reading any quality comparison, check what was controlled. Model size, training data volume, and compute budget all affect benchmark scores. Most earlier comparisons did not control all three at once. Vicentino (2026), [arXiv:2603.22075](https://arxiv.org/abs/2603.22075), is a particularly useful controlled comparison because it matches model size, training data, and compute. It should be treated as one careful reference point, not as the final word for larger-scale regimes.
...
- Reasoning benchmark results that control for model size and training compute. Vicentino (2026, [arXiv:2603.22075](https://arxiv.org/abs/2603.22075)) is a useful example of that style of evaluation.
